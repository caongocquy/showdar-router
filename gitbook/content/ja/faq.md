...
Showdar Routerに関する一般的な質問。

---

## Showdar Routerとは?

**Showdar Routerは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
## Showdar Routerは無料ですか?

**はい、Showdar Router自体は100%無料でオープンソースです。**
...
**Showdar Routerはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## Showdar Routerをセルフホストできますか?

**はい! Showdar Routerは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.showdar-router`)
...
**はい、Showdar Routerはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.showdar-router`(またはカスタム`DATA_DIR`)にローカル保存
- Showdar Routerサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**Showdar Routerが保存するもの:**
- プロバイダーOAuthトークン(暗号化)
- APIキー(暗号化)
- 使用統計(ローカルのみ)
- コンボ設定

**Showdar Routerが保存しないもの:**
- プロンプトやレスポンス
- 生成したコード
- 個人情報
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/caongocquy/showdar-router/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.showdar-router`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/caongocquy/showdar-router/blob/main/CONTRIBUTING.md)を参照。
