...
将 Showdar Router 与 Cline VSCode 扩展集成,通过 Showdar Router 的智能路由系统转发你的 AI 请求。
...
将 base URL 设为你的 Showdar Router endpoint:
...
**步骤:**
1. 在 **Base URL** 字段中输入你的 Showdar Router endpoint
2. 末尾必须包含 `/v1`
...
你可以使用 Showdar Router 仪表盘中配置的任意模型。常见示例:
...
2. 在聊天输入框中输入消息
3. 按 Enter 发送
4. Cline 会通过 Showdar Router 处理你的请求

### 生成代码

1. 让 Cline 生成代码:"创建一个登录表单的 React 组件"
2. Cline 会通过 Showdar Router 生成代码
3. 检查并接受生成的代码
...
1. 在编辑器中选中代码
2. 让 Cline:"解释这段代码"
3. 通过 Showdar Router 获得 AI 驱动的解释

### 文件操作

1. 让 Cline 创建、修改或删除文件
2. Cline 会通过 Showdar Router 理解上下文并进行修改
3. 在接受前检查变更
...
1. 确认 Showdar Router 正在运行:`curl http://localhost:21298/health`
2. 确认 base URL 正确且包含 `/v1`
3. 确保防火墙没有阻止 21298 端口
4. 尝试重启 VSCode
...
1. 确认模型名与 Showdar Router 配置完全一致
...
2. 确认 Showdar Router 实例正在运行且健康
3. 重新加载 VSCode 窗口(Cmd/Ctrl + Shift + P → "Reload Window")
4. 检查 Showdar Router 日志是否有错误
...
2. **监控使用**:在 Showdar Router 仪表盘查看用量统计和成本
3. **管理上下文**:保持对话聚焦以减少 token 用量
4. **切换模型**:根据任务复杂度切换模型,优化成本和性能
5. **API Key 安全**:绝不将 API key 提交到版本控制

## 与 Showdar Router 功能的集成

### 模型路由

Showdar Router 会根据以下因素自动将请求路由到最佳提供商:
- 模型可用性
- 提供商健康状态
- 成本优化
- 负载均衡
...
某个提供商失败时,Showdar Router 会自动回退到仪表盘中配置的备用提供商。

### 使用跟踪

通过 Showdar Router 仪表盘监控你的 Cline 使用:
- 请求总数
- Token 使用
- 每个模型的成本
- 提供商分布
