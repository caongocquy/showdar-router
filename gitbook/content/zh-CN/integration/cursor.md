...
将 Showdar Router 与 Cursor IDE 集成,通过 Showdar Router 的智能路由系统转发你的 AI 请求。

## 前置要求

- 已安装 Cursor IDE
- Cursor Pro 账户(使用自定义 API endpoint 必需)
- 已配置 Showdar Router 云端 endpoint
...
3. 输入 Showdar Router 配置中的模型名(例如 `gpt-4`、`claude-opus-4-5` 等)
4. 点击 **Add**
...
2. 从列表中选择你的自定义模型
3. 开始在 Cursor 中使用 Showdar Router!
...
2. 按 Cmd/Ctrl + K
3. 输入 prompt
4. Cursor 会通过 Showdar Router 生成代码
...
4. 通过 Showdar Router 获得 AI 驱动的解释
...
如果你在本地运行 Showdar Router 并希望搭配 Cursor 使用:

1. 在 Showdar Router 设置中启用云端 endpoint
2. 在 Showdar Router 仪表盘中配置云端 endpoint URL
3. 在 Cursor 设置中使用该云端 URL
4. 确保本地 Showdar Router 实例可从互联网访问

## 最佳实践

1. **使用模型别名**:为常用模型在 Showdar Router 中创建简短别名
