...
将 Showdar Router 与 Roo AI 助手集成,通过统一界面访问多个 AI 模型。
...
### 连接失败
- 确认 Showdar Router 正在运行:`curl http://localhost:21298/health`
- 检查 API key 是否正确
- 确保 Base URL 末尾包含 `/v1`

### 模型不可用
- 检查模型名是否完全匹配(大小写敏感)
- 确认 Showdar Router 套餐中已启用该模型
- 尝试列表中的其他模型

### 响应缓慢
- 切换到更快的模型(haiku、flash)
- 检查网络连接
- 查看 Showdar Router 日志排查问题
