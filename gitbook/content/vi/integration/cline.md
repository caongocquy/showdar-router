...
Tích hợp Showdar Router với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của Showdar Router.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- Showdar Router đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint Showdar Router:
...
**Các bước:**
1. Trong field **Base URL**, nhập endpoint Showdar Router
...
1. Trong dropdown **Model**, bạn có thể:
   ...
   - Nhập tên model thủ công từ cấu hình Showdar Router
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng Showdar Router để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua Showdar Router
...
3. Nhận giải thích AI qua Showdar Router

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng Showdar Router để hiểu context và thực hiện thay đổi
...
2. Xác minh Showdar Router instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs Showdar Router để xem lỗi
...
## Tích hợp với Tính năng Showdar Router

### Định tuyến Model

Showdar Router tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, Showdar Router tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua Showdar Router dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
