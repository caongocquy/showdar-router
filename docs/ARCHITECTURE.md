...
Showdar Router is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Showdar Router Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.showdar-router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.showdar-router/usage.json`, `~/.showdar-router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.showdar-router/db.json`)
- usage stats: `~/.showdar-router/usage.json`
- request log lines: `~/.showdar-router/log.txt`
...
1. `usageDb` currently stores under `~/.showdar-router` and does not follow `DATA_DIR`.
