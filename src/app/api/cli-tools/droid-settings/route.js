...
// Check if settings has Showdar Router customModels
const has9RouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Showdar Router"));
};
...
// POST - Update Showdar Router customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Showdar Router configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Showdar Router"));
...
// DELETE - Remove Showdar Router customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Showdar Router customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Showdar Router"));
