...
// Collect PIDs of all showdar-router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from showdar-router install dir or wrapper cli.js
        const isAppProcess = lower.includes("showdar-router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("showdar-router") ||
...
// Resolve npx/showdar-router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx showdar-router` — works regardless of global bin path changes after npm i -g
