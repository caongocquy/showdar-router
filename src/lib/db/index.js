...
export async function exportDb() {
  ...
  for (const r of db.all(`SELECT key, value FROM kv WHERE scope = 'routeHealth'`)) out.routeHealth[r.key] = parseJson(r.value);
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    db.run(`DELETE FROM kv WHERE scope IN ('modelAliases', 'customModels', 'mitmAlias', 'pricing', 'routeHealth')`);
    ...
    for (const [model, record] of Object.entries(payload.routeHealth || {})) {
      db.run(`INSERT OR REPLACE INTO kv(scope, key, value) VALUES('routeHealth', ?, ?)`, [model, stringifyJson(record)]);
    }
  });
