...
export function applyOutboundProxyEnv(
  ...
  if (!enabled) {
    if (process.env.SHOWDAR_ROUTER_PROXY_MANAGED === "1" || process.env.NINE_ROUTER_PROXY_MANAGED === "1") {
      delete process.env.HTTP_PROXY;
      delete process.env.HTTPS_PROXY;
      delete process.env.ALL_PROXY;
      delete process.env.NO_PROXY;
      delete process.env.SHOWDAR_ROUTER_PROXY_MANAGED;
      delete process.env.SHOWDAR_ROUTER_PROXY_URL;
      delete process.env.SHOWDAR_ROUTER_NO_PROXY;
  ...
  const wasManaged = process.env.SHOWDAR_ROUTER_PROXY_MANAGED === "1" || process.env.NINE_ROUTER_PROXY_MANAGED === "1";
  let managed = false;
  ...
  if (proxyUrl) {
    ...
    if (validated) {
      ...
      process.env.SHOWDAR_ROUTER_PROXY_URL = validated;
      managed = true;
    }
  }
  ...
  if (noProxy) {
    ...
    process.env.SHOWDAR_ROUTER_NO_PROXY = noProxy;
    managed = true;
  }

  if (managed) {
    process.env.SHOWDAR_ROUTER_PROXY_MANAGED = "1";
  } else if (wasManaged) {
