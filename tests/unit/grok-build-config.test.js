...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("showdar-router");
    expect(parsed.model).toMatchObject({
      model: "cx/gpt-5.6-sol",
      base_url: "http://127.0.0.1:21298/v1",
      context_window: 400000,
    });
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "showdar-router-general-purpose",
      explore: "showdar-router-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.showdar-router\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.showdar-router-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.showdar-router-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# showdar-router-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.showdar-router-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("showdar-router-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.showdar-router-");
    expect(reset).not.toContain("showdar-router-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("showdar-router-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("showdar-router-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("showdar-router-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("showdar-router-explore");
    expect(getGrokSubagentSlot("plan")).toBe("showdar-router-plan");
