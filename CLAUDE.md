...
cp .env.example .env
npm install
PORT=21298 NEXT_PUBLIC_BASE_URL=http://localhost:21298 npm run dev   # dev (webpack, port 20127 by default via next dev)
npm run build && PORT=21298 HOSTNAME=0.0.0.0 npm run start           # production
```
- Bun variants: `npm run dev:bun` / `build:bun` / `start:bun`.
- Default runtime port is **21298** (dashboard at `/dashboard`, API at `/v1`).
...
- DB file location resolves via `src/lib/db/paths.js` (`DATA_DIR`, else `~/.showdar-router/`).
- Usage/logs (`src/lib/usageDb.js`, `usage.json` + `log.txt`) still live under `~/.showdar-router` and do **not** follow `DATA_DIR`.
