...
function createRequestContextResponse() {
  // AgentService asks every run for client context. Showdar Router has no IDE file
      ...
      try {
        while (!finished) {
          ...
          pending = decodeAgentFrames(pending, (payload) => {
            ...
            // Return an empty context; Showdar Router is not coupled to an editor.
            if (serverMessage.has(2)) {
              ...
              if (execRequest.has(10)) {
                ...
                // (shell, read, write, …) that Showdar Router cannot service. Fail the
