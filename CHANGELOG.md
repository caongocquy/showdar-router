...
- **CLI tools**: send the API key Codex actually reads —
  `[model_providers.showdar-router.http_headers]` instead of auth.json (which left
...
- **Providers**: add Self-hosted STT / TTS / Embedding — point Showdar Router at your own
...
- **RTK**: `X-Showdar Router-Token-Saver` header to bypass token savers per request
...
## Docs
- README: add Indonesian Showdar Router tutorial video (#1709)
