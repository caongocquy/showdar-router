...
async function buildCodexHeader() {
  ...
  const { installed, hasShowdarRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasShowdarRouter, settings } = result.data;
  ...
  // Extract Showdar Router custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Showdar Router-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Showdar Router provider config
  const provider = settings?.models?.providers?.["showdar-router"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("showdar-router/") ? primary.replace("showdar-router/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasShowdarRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Showdar Router\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
