...
function ensureDaemon({ daemon, appRoot, env }) {
...
function runTray({ daemon, tray, appRoot, env = process.env, cliPath = path.resolve(__dirname, "..", "cli.js") }) {
  const port = Number(env.SHOWDAR_ROUTER_PORT || env.PORT || daemon.DEFAULT_PORT || DEFAULT_PORT);
  const current = ensureDaemon({ daemon, appRoot, env: { ...env, SHOWDAR_ROUTER_PORT: String(port) } });
  const options = {
    port,
    running: true,
    ...
    onRestart: () => {
      ...
      ensureDaemon({ daemon, appRoot, env: { ...env, SHOWDAR_ROUTER_PORT: String(port) } });
    },
