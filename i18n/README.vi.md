<div align="center">
  <img src="../images/9router.png?1" alt="Bảng điều khiển Showdar Router" width="800"/>
...
**Showdar Router giải quyết vấn đề này:**
...
🎉 Bảng điều khiển (Dashboard) sẽ tự động mở tại `http://localhost:20129`
...
URL mặc định:
- Bảng điều khiển Dashboard: `http://localhost:20129/dashboard`
- API tương thích OpenAI: `http://localhost:20129/v1`
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - Showdar Router + Claude Code MIỄN PHÍ
  
[![Thiết lập Showdar Router + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập Showdar Router
...
Showdar Router hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → Showdar Router dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân Showdar Router **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của Showdar Router

**Thực tế Thanh toán Showdar Router:**

✅ **Phần mềm Showdar Router = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **Showdar Router không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, Showdar Router chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị Showdar Router tính phí không?</b></summary>

**Không.** Showdar Router là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, Showdar Router chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân Showdar Router** → **Không bao giờ tính phí bất cứ thứ gì, ever**

Showdar Router là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
Showdar Router chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và Showdar Router giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải Showdar Router.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - Showdar Router giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của Showdar Router ngăn chặn các khoản phí bất ngờ:

**Kịch bản:** Bạn đang trong giai đoạn code nước rút và vượt qua các hạn mức

**Không có Showdar Router:**
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và Showdar Router sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. Showdar Router theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển showdar-router]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với Showdar Router cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.showdar-router/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.showdar-router` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
| `PORT` | framework default | Cổng dịch vụ (`20129` trong các ví dụ) |
| `HOSTNAME` | framework default | Bind host (Docker mặc định là `0.0.0.0`) |
| `NODE_ENV` | runtime default | Đặt `production` để triển khai |
| `BASE_URL` | `http://localhost:20129` | URL cơ sở nội bộ phía máy chủ được sử dụng bởi các tác vụ đồng bộ đám mây |
...
- Lịch sử sử dụng và log: `~/.showdar-router/usage.json` và `~/.showdar-router/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.showdar-router` và độc lập với `DATA_DIR`.
...
**Token OAuth hết hạn**
- Tự động làm mới bởi Showdar Router
...
- Đặt `PORT=20129` và `NEXT_PUBLIC_BASE_URL=http://localhost:20129`

**Lỗi đồng bộ đám mây**
- Xác minh `BASE_URL` trỏ đến phiên bản đang chạy của bạn (ví dụ: `http://localhost:20129`)
...
- Runtime Showdar Router cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- cục bộ (`http://127.0.0.1:20129/v1/chat/completions`): hoạt động với `stream=false` và `stream=true`.
...
Cảm ơn tất cả những người đã đóng góp giúp Showdar Router tốt hơn!
